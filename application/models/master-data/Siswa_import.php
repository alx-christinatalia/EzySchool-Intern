<?php
	...
	class Siswa_import extends CI_Model {
		...
		public function GetDatasiswaimport($param) {
			$args = [
		...
		public function HtmlDatasiswa($param) {
			...
			$query = $this->GetDatasiswaimport($param);
				...
				if(!empty($query->Data)) {
					foreach ($query->Data as $item) {
						...
						$rHtml .= '<tr>
										...
										<td><a href="'.base_url("siswa/edit.html?nis=".$item->nis).'">'.$item->nis.'</a></td>
										<td>'.$item->nisn.'</td>
										<td>'.$item->nama.'</td>
