<?php
	...
	class Mesin_sidik_jari extends CI_Model {
		...
		public function GetMesin($param) {
			...
			$this->api->set("master-data/mesin/finger/list", $args);
				...
				if(!empty($query->Data)) {
					foreach ($query->Data as $item) {
						...
						$rHtml .= '<tr>
                                        ...
                                        <td>'.$item->jenjang.'</td>
                                        <td>'.$item->email.'</td>
                                        <td>'.$item->uid_mesin_server.'</td>
                                        <td><a role="button" class="edit-data" data-idupdate="'.$item->id.'">'.$item->uid_mesin_finger.'</td>
		...
		public function InsertMesin($data, $param) {
        	...
        	$param['db'] = $this->db;
            $param['id_mesin_server'] = $data['id_mesin_server'];
        	$param['uid_mesin_finger'] = $data['uid_mesin_finger'];
        	$param['catatan'] = $data['catatan'];

           	$this->api->set("master-data/mesin/finger/add", $param);
			$query = $this->api->exec();
        ...
        public function UpdateMesin($id_update, $param) {
        	...
        	// $param['id_sekolah'] = $id_update['id_sekolah'];
        	// $param['id_mesin_server'] = $id_update['id_mesin_server'];
        	// $param['id_mesin_finger'] = $id_update['id_mesin_finger'];

           	$this->api->set("master-data/mesin/finger/edit", $param);
			$query = $this->api->exec();
			if($query->IsError == true) {
				$dataError = preg_match_all("!Parameter[\s+]\'([\w]+)!", $query->ErrMessage, $matches);
				if($dataError) {
