...
class Ajax_kategori_tagihan extends CI_Controller {
    ...
    private function listdatahtml() {
        ...
        $Request = $this->kategori_tagihan->HtmlKategori($this->req);
        return $Request;
    }

    private function listdropdown() {
        $this->req["filter"]["status"] = 1;
        $Request = $this->kategori_tagihan->HtmlDropdownKategori($this->req);
        return $Request;
    }

    private function getdatabyid() {
        $Request = $this->kategori_tagihan->GetKategori(["filter" => ["id" => $this->req]]);
        return json_encode($Request);
    }

    private function insertdata() {
        $Request = $this->kategori_tagihan->InsertKategori($this->form);
        return $Request;
    }

    private function updatedata() {
        $id_update = $this->form["id_update"]; unset($this->form["id_update"]);
        $Request = $this->kategori_tagihan->UpdateKategori($id_update, $this->form);
        return $Request;
    }
}
