...
<html lang="en">
    ...
    <head>
        <meta charset="utf-8" />
        <title>Ganti Password | EzySchool Sekolah</title>
    ...
    <body class="login">
        ...
        <div class="content">
            <h3 class="form-title font-green">Ganti Password</h3>
            ...
            <form id="FrmLogin" role="form">
                <div class="form-group">
                    ...
                    <input type="password" class="form-control form-control-solid placeholder-no-fix newpass1" placeholder="Password Baru" autofocus required minlength="6">
                    <input type="hidden" name="form[newpass1]">
                </div>
                <div class="form-group">
                    <label class="control-label visible-ie8 visible-ie9">Tulis Ulang Password Baru</label>
                    <input type="password" class="form-control form-control-solid placeholder-no-fix newpass2" placeholder="Tulis Ulang Password Baru" required minlength="6">
                    <input type="hidden" name="form[newpass2]">
                </div>
                <div class="form-group recaptcha">
                    <?php echo "<center>".$this->recaptcha->getWidget()."</center>"; ?>
                </div>
                <div class="form-actions">
                    <input type="hidden" value="<?php echo $this->uri->segment(3); ?>" name="form[id_sekolah]">
                    <input type="hidden" value="<?php echo $this->uri->segment(4); ?>" name="form[user_key]">
                    <button type="submit" class="btn green ladda-button ladda-button-submit" data-style="slide-up">Ganti Password</button>
        ...
        <script>
            ...
            $(".newpass1").keyup(function() {
                $("input[name='form[newpass1]']").val($.md5($(this).val()));
            });

            $(".newpass2").keyup(function() {
                $("input[name='form[newpass2]']").val($.md5($(this).val()));
            });

            var FrmSave = $("#FrmLogin").validate({
                submitHandler: function(form) {
                    ResetPass(form, function(resp) {
